...
#### extract race characteristics from 2019 American Community Survey - Demographic and Housing Estimates 
...
ACS_race<-read.csv("ACSDP5Y2019.DP05_data_with_overlays_2021-05-12T022242.csv", header=TRUE)
...
ACS_race_short<-dplyr::select(ACS_race, DP05_0077PE, DP05_0078PE, DP05_0079PE, DP05_0080PE, DP05_0081PE, DP05_0082PE, DP05_0083PE)
...
ACS_race_short$hispanic_zipcode<-ACS_race2$Hispanic
...
Data<-read.csv("BADR_pilot_4to6months_neighborhood_zipcode.csv")
...
ACS_nonenglish<-read.csv("ACSST5Y2019.S1601_data_with_overlays_2021-05-20T222030.csv", header=TRUE)
...
# extract columns of interest (the % of population in that language)
...
ACS_income<-read.csv("ACSDP5Y2019.DP03_data_with_overlays_2021-05-20T223234.csv", header=TRUE)
...
ACS_ed<-read.csv("ACSST5Y2019.S1501_data_with_overlays_2021-05-20T223931.csv", header=TRUE)
...
write.csv(cc_combined, file = "BADR_pilot_4to6months_census.csv")
...
for (i in  1:nrow(Data)) {
  ...
  {
    Data[i,match("white_zipcode",names(Data)):ncol(Data)]<-subset(cc_combined_short, cc_combined_short$zipcode_final==as.character(Data[i,match("zipcode_final",names(Data))]))
  }
  else{}
}
...
# write data
write.csv(Data, file = "BADR_pilot_4to6months_census_and_neighborhoodtrust.csv")
